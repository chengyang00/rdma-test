...
static struct ibv_mr *client_metadata_mr = NULL, *server_buffer_mr = NULL, *server_metadata_mr = NULL, *server_recv_mr = NULL;
static struct rdma_buffer_attr client_metadata_attr, server_metadata_attr;
static struct ibv_recv_wr client_recv_wr, *bad_client_recv_wr = NULL;
static struct ibv_send_wr server_send_wr, *bad_server_send_wr = NULL;
static struct ibv_recv_wr server_recv_wr, *bad_server_recv_wr = NULL;
static struct ibv_sge client_recv_sge, server_send_sge, server_recv_sge;
static char *buffer = NULL;
static uint8_t tos = 2;
static unsigned int size = 4096;
static unsigned int iters = 1000;
/* socket resources */
int sockfd = 0, peer_sockfd = 0;
struct sockaddr_in peer_addr;
socklen_t peer_addr_len = sizeof(struct sockaddr_in);
char sock_buf[64] = {'\0'};

/* connect to client with socket */
static int connect_with_socket(struct sockaddr_in *server_addr)
{
	// char port_str[20];
 ...
 * create connection identifier, bind it to server_addr, listen on the passed IP and port
 * and get the connection request event of a client.
 */
static int start_rdma_server(struct sockaddr_in *server_addr)
{
	 ...
	 * RDMA_PS_TCP provides reliable, connection-oriented QP communication.
	 */
	...
	ret = rdma_set_option(cm_client_id, RDMA_OPTION_ID, RDMA_OPTION_ID_TOS, &tos, sizeof tos);
	if (ret)
	{
		rdma_error("Failed to set the traffic class, errno: %d\n", -errno);
		return -errno;
	}
 ...
 * RDMA credentials.
 * allocate pd, create completion channel, create cq and create q
 */
static int setup_client_resources()
{
	 ...
	 * The capacity here is defined statically but this can be probed from the
	 * device. We just use a small number as defined in rdma_common.h */
	bzero(&qp_init_attr, sizeof qp_init_attr);
	qp_init_attr.cap.max_recv_sge = MAX_SGE; /* Maximum SGE number per receive posting */
	qp_init_attr.cap.max_recv_wr = MAX_WR;	 /* Maximum receive posting capacity */
	qp_init_attr.cap.max_send_sge = MAX_SGE; /* Maximum SGE number per send posting */
...
static int recv_op()
{
	...
	server_recv_sge.addr = (uint64_t)server_recv_mr->addr;
	server_recv_sge.length = (uint32_t)server_recv_mr->length;
	server_recv_sge.lkey = server_recv_mr->lkey;
...
static int disconnect_and_cleanup()
{
	...
	rdma_buffer_deregister(server_recv_mr);
...
{
	printf("Usage:\n");
	printf("rdma_server: [-a <server_addr>] [-p <server_port>] [-t <tos>]\n");
...
int main(int argc, char **argv)
{
	...
	/* allow sin_addr to accept incoming connections on all network interfaces */
	server_sockaddr.sin_addr.s_addr = htonl(INADDR_ANY);
	/* Parse Command Line Arguments, not the most reliable code */
	while ((option = getopt(argc, argv, "a:p:s:n:t:")) != -1)
	{
		switch (option)
		{
		case 'a':
	...
	if (buffer == NULL)
	{
		rdma_error("Failed to allocate memory for buffer!");
		return ret;
	}
	...
	ret = connect_with_socket(&server_sockaddr);
	if (ret)
	{
		rdma_error("RDMA server failed to connect to client with socket, ret = %d\n", ret);
		return ret;
	}
