...
struct __attribute((packed)) rdma_buffer_attr {
  ...
  union stag {
	  ...
	  uint32_t local_stag;  /* lkey */
	  /* if we receive, we call it remote stag */
	  uint32_t remote_stag; /* rkey */
  }stag;
};
...
/* 
 * Processes an RDMA connection management (CM) event. Get a communication event from echannel and check its correctness.
...
/* Allocates an RDMA buffer of size 'length' with permission. This 
...
/* create a socket and establish a connection to the port of the specified server.
