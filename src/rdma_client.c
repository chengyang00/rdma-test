...
/* socket connection sources */
int peer_sockfd = 0;
char sock_buf[64] = SOCK_SYNC_MSG;

/* connect to server with socket */
static int connect_with_socket(struct sockaddr_in *s_addr)
{
	// char port_str[20];
...
static int client_prepare_connection(struct sockaddr_in *s_addr)
{
	 ...
	 */
	io_completion_channel = ibv_create_comp_channel(cm_client_id->verbs); // verbs refers to device
...
{
	int ret = -1;
	/* receive server information(the memory region info of server) and write it into server_metadata_attr. the address of server_matadata_attr is buffer */
...
static int send_op()
{
	...
	if (ret)
	{
		rdma_error("Failed to post the send request, errno: %d \n", ret);
		return ret;
	}
...
int main(int argc, char **argv)
{
	...
	server_sockaddr.sin_family = AF_INET;					  // IPv4 address family
	server_sockaddr.sin_addr.s_addr = htonl(INADDR_LOOPBACK); // local host network interface
	...
	if (ret)
	{
		rdma_error("RDMA client failed to connect to server with socket, ret = %d\n", ret);
		return ret;
	}
